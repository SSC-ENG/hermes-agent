...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "You already have board/task state from your initial `kanban_show()` at "
    "session start. Do NOT re-`kanban_show()` the full board again unless you "
    "have created or blocked a card since your last snapshot. To check a single "
    "other task, use `kanban_show(task_id=...)` for a targeted read, never a "
    "full-board dump. Prefer `read_file` `offset`/`limit` ranged reads over "
    "full-file reads, especially for large files or long-running/looping "
    "sessions.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
